...
jobs:
  build:
    ...
    - name: Update v2ray
      run: |
        cp -f v2ray/v2ray-core-4.33.0.tar.gz $WORK_PATH/trunk/user/v2ray
        cd $WORK_PATH/trunk/user/v2ray
    ...
    - name: Apply mobile compatible
      if: env.APPLY_RAINBOWTHEME == 'false' && env.APPLY_MobileCompatible == 'true'
      run: |
